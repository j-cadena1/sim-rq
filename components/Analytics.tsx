    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-400 mt-1">Insights and performance metrics</p>
        </div>
      ...
      {activeTab === 'overview' && dashboardStats && (
        <div className="space-y-6">
          ...
          {dashboardStats.averageMetrics && (
            <div className="bg-slate-900 rounded-lg shadow-sm p-6 border border-slate-800">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  <p className="text-sm text-slate-400 mb-1">Avg. Hours per Request</p>
                  <p className="text-2xl font-bold text-white">
          ...
          {dashboardStats.requestsByStatus && dashboardStats.requestsByStatus.length > 0 && (
            <div className="bg-slate-900 rounded-lg shadow-sm p-6 border border-slate-800">
              <h2 className="text-xl font-bold text-white mb-4">Requests by Status</h2>
              <div className="space-y-3">
                {dashboardStats.requestsByStatus.map((item) => (
                  <div key={item.status} className="flex items-center gap-4">
                    <div className="w-32 text-sm font-medium text-slate-300">{item.status}</div>
          ...
          {dashboardStats.requestsByPriority && dashboardStats.requestsByPriority.length > 0 && (
            <div className="bg-slate-900 rounded-lg shadow-sm p-6 border border-slate-800">
              <h2 className="text-xl font-bold text-white mb-4">Requests by Priority</h2>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {dashboardStats.requestsByPriority.map((item) => {
                  ...
                  return (
                    <div
                      key={item.priority}
                      className={`p-4 rounded-lg border ${colors[item.priority as keyof typeof colors] || 'bg-slate-800 text-slate-300 border-slate-700'}`}
                    >
          ...
          {dashboardStats.projectUtilization && dashboardStats.projectUtilization.length > 0 && (
            <div className="bg-slate-900 rounded-lg shadow-sm p-6 border border-slate-800">
              <h2 className="text-xl font-bold text-white mb-4">Project Utilization</h2>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {dashboardStats.projectUtilization.slice(0, 10).map((project) => (
                      <tr key={project.projectId} className="border-b border-slate-800">
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <div className="flex-1 bg-slate-800 rounded-full h-4 overflow-hidden max-w-[100px]">
                              <div
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {dashboardStats.engineerWorkload.map((engineer) => (
                      <tr key={engineer.engineerId} className="border-b border-slate-800">
          ...
          {dashboardStats.topVendors && dashboardStats.topVendors.length > 0 && (
            <div className="bg-slate-900 rounded-lg shadow-sm p-6 border border-slate-800">
              <h2 className="text-xl font-bold text-white mb-4">Top Vendors by Requests</h2>
              <div className="space-y-3">
                {dashboardStats.topVendors.slice(0, 5).map((vendor, index) => (
                  <div key={vendor.vendor} className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="text-sm font-medium text-white">{vendor.vendor}</div>
...
const CompletionTimeTab: React.FC<{ data: any; isLoading: boolean }> = ({ data, isLoading }) => {
  if (isLoading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-slate-400">Loading completion analysis...</p>
      </div>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-slate-900 rounded-lg shadow-sm p-12 border border-slate-800 text-center">
        <Clock className="w-16 h-16 text-slate-700 mx-auto mb-4" />
        <p className="text-slate-400">No completion data available yet.</p>
      </div>
    );
  }
    ...
    <div className="bg-slate-900 rounded-lg shadow-sm p-6 border border-slate-800">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-700">
              <th className="text-left py-3 px-4 text-sm font-semibold text-slate-300">Priority</th>
              <th className="text-right py-3 px-4 text-sm font-semibold text-slate-300">
                Requests
              </th>
              <th className="text-right py-3 px-4 text-sm font-semibold text-slate-300">
          ...
          <tbody>
            {data.map((row: any) => (
              <tr key={row.priority} className="border-b border-slate-800">
                <td className="py-3 px-4">
                  <span
...
const AllocationAnalysisTab: React.FC<{ data: any; isLoading: boolean }> = ({
  ...
  if (isLoading) {
    return (
      <div className="text-center py-12">
        <div className="w-12 h-12 border-4 border-blue-600/30 border-t-blue-600 rounded-full animate-spin mx-auto mb-4"></div>
        <p className="text-slate-400">Loading allocation analysis...</p>
      </div>
    );
  }
    ...
    <div className="bg-slate-900 rounded-lg shadow-sm p-6 border border-slate-800">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-700">
              <th className="text-left py-3 px-4 text-sm font-semibold text-slate-300">Request</th>
              <th className="text-left py-3 px-4 text-sm font-semibold text-slate-300">Priority</th>
              <th className="text-right py-3 px-4 text-sm font-semibold text-slate-300">
                Allocated
              </th>
              <th className="text-right py-3 px-4 text-sm font-semibold text-slate-300">Actual</th>
              <th className="text-right py-3 px-4 text-sm font-semibold text-slate-300">
                Variance
              </th>
              <th className="text-right py-3 px-4 text-sm font-semibold text-slate-300">
          ...
          <tbody>
            {data.map((row: any) => (
              <tr key={row.requestId} className="border-b border-slate-800">
